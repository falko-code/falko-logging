...
public readonly partial struct Logger
{
    ...
    public void Warn(DefaultInterpolatedStringHandler messageHandler)
    {
        var loggerContext = _loggerRuntime.LoggerContext;

        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn, messageHandler.ToStringAndClear());
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public void Warn(Exception? exception, DefaultInterpolatedStringHandler messageHandler)
    {
        var loggerContext = _loggerRuntime.LoggerContext;

        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn, exception, messageHandler.ToStringAndClear());
        }
    }
    ...
    public void Warn([Localizable(false)][StructuredMessageTemplate] string? message)
    {
        ...
        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn, message);
        }
    }
    ...
    public void Warn(Exception? exception, [Localizable(false)][StructuredMessageTemplate] string? message)
    {
        ...
        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn, exception, message);
        }
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public void Warn<T>(T messageArgument, LogMessageFactory<T> messageFactory)
    {
        var loggerContext = _loggerRuntime.LoggerContext;

        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn,
                messageArgument, messageFactory);
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public void Warn<T>(Exception? exception, T messageArgument, LogMessageFactory<T> messageFactory)
    {
        var loggerContext = _loggerRuntime.LoggerContext;

        if (loggerContext.IsWarnLevelEnabled)
        {
            Log(in loggerContext, LogLevel.Warn, exception,
                messageArgument, messageFactory);
        }
    }
